...
export default function AdminPage(){
  const[u,setU]=useState<any[]>([]);const[e,setE]=useState("");const[c,setC]=useState("USD");const[a,setA]=useState("");const[msg,setMsg]=useState("");const[er,setEr]=useState("")
        ...
        <div className="g2" style={{gap:24}}>
          <div className="card" style={{padding:24}}>
            ...
            <form onSubmit={fd} style={{display:"flex",flexDirection:"column",gap:12}}>
              <select value={e} onChange={e2=>setE(e2.target.value)} className="inp" required>
                <option value="">Select user...</option>
                {u.filter((x:any)=>x.role!=="ADMIN").map((x:any)=><option key={x.id} value={x.email}>{x.name} ({x.email})</option>)}
              </select>
              <div style={{display:"flex",gap:8}}>
                <select value={c} onChange={e2=>setC(e2.target.value)} className="inp" style={{width:100}}>{CC.map(x=><option key={x} value={x}>{x}</option>)}</select>
                <input type="number" step="0.01" min="0.01" value={a} onChange={e2=>setA(e2.target.value)} placeholder="Amount" className="inp" style={{flex:1}} required/>
              </div>
              <button type="submit" className="btn btn-p" style={{width:"100%"}}>💫 Deposit</button>
            </form>
          </div>
          <div className="card" style={{padding:24}}>
            <h3 style={{fontSize:16,fontWeight:600,marginBottom:16}}>👥 Users ({u.length})</h3>
