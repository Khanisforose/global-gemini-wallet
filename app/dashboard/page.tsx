...
export default function Dashboard() {
  ...
  const sendFunds = async(e:any) => { e.preventDefault(); setSM(""); setSE(""); try { const r=await fetch("/api/transfer",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({toEmail,currency:sendCurr,amount:parseFloat(sendAmt)})}); const d=await r.json(); if(!r.ok) throw new Error(d.error); setSM(`✅ Sent ${sendAmt} ${sendCurr} to ${toEmail}`); setSA(""); setTE(""); } catch(e:any) { setSE(e.message); } };
  ...
  const BG = "rgba(255,255,255,0.03)"; const BR = "1px solid rgba(255,255,255,0.06)"; const IP = {width:"100%",padding:"12px 14px",background:"#1a1a2e",border:BR,borderRadius:"10px",color:"#fff",fontSize:"14px",outline:"none"};
    ...
    <div style={{minHeight:"100vh",background:"#0a0a0f",color:"#fff",fontFamily:"Inter,sans-serif"}}>
      <div style={{borderBottom:BR,padding:"14px 32px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"16px"}}>
          {["wallet","profile","refer","settings"].map(t=>(<button key={t} onClick={()=>setT(t)} style={{fontSize:"13px",padding:"6px 0",background:"none",border:"none",borderBottom:tab===t?"2px solid #d4af37":"2px solid transparent",color:tab===t?"#d4af37":"#6b7280",cursor:"pointer",textTransform:"capitalize"}}>{t==="wallet"?"💰 Wallet":t==="profile"?"👤 Profile":t==="refer"?"🔗 Refer":"⚙️ Settings"}</button>))}
      ...
      <div style={{maxWidth:"1400px",margin:"0 auto",padding:"24px 32px"}}>
        {tab==="wallet" && (
          <>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"24px",flexWrap:"wrap",gap:"12px"}}>
              <div><h1 style={{fontSize:"26px",fontWeight:"bold",fontFamily:"Georgia,serif"}}>Welcome, {user.name}</h1><p style={{fontSize:"34px",fontWeight:"bold",background:"linear-gradient(135deg,#d4af37,#f0d060)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",marginTop:"4px"}}>{fmt(total)} <span style={{fontSize:"14px",color:"#6b7280",fontWeight:"400",WebkitTextFillColor:"#6b7280"}}>total USD</span></p></div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"20px",marginBottom:"20px"}}>
              ...
              <div style={{background:BG,backdropFilter:"blur(24px)",border:BR,borderRadius:"16px",padding:"20px"}}>
                <h2 style={{fontSize:"16px",fontWeight:"600",fontFamily:"Georgia,serif",marginBottom:"12px"}}>🪙 Crypto Portfolio</h2>
                {[{s:"USDT",b:1250.5,c:"#26a17b",usd:1250.5},{s:"BTC",b:0.025,c:"#f7931a",usd:1675},{s:"ETH",b:0.5,c:"#627eea",usd:1700},{s:"SOL",b:10,c:"#9945ff",usd:1450},{s:"BNB",b:2.3,c:"#f3ba2f",usd:1380}].map(x=><div key={x.s} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:BR}}><div><span style={{fontWeight:"500",color:x.c}}>{x.s}</span><span style={{marginLeft:"8px",fontFamily:"Georgia,serif"}}>{x.b.toLocaleString("en-US",{minimumFractionDigits:4})}</span></div><span style={{color:"#d4af37"}}>{fmt(x.usd)}</span></div>)}
              </div>
            </div>

            {kyc==="VERIFIED" && (
              <div style={{background:BG,backdropFilter:"blur(24px)",border:BR,borderRadius:"16px",padding:"20px",marginBottom:"20px"}}>
                <h2 style={{fontSize:"16px",fontWeight:"600",fontFamily:"Georgia,serif",marginBottom:"12px"}}>📤 Send Funds</h2>
                ...
                <form onSubmit={sendFunds} style={{display:"flex",gap:"8px",flexWrap:"wrap"}}>
                  <input type="email" value={toEmail} onChange={e=>setTE(e.target.value)} placeholder="Recipient email" style={{...IP,flex:1,minWidth:"200px"}} required />
                  ...
                  <select value={sendCurr} onChange={e=>setSC(e.target.value)} style={{...IP,width:"80px"}}><option>USD</option><option>EUR</option><option>GBP</option><option>USDT</option></select>
            ...
            <div style={{background:BG,backdropFilter:"blur(24px)",border:BR,borderRadius:"16px",padding:"20px"}}>
              ...
              {txs.length===0?<p style={{color:"#6b7280",fontSize:"13px",textAlign:"center",padding:"16px"}}>No transactions yet</p>:txs.slice(0,25).map((tx:any)=>(
                <div key={tx.id} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:BR}}>
                  <div><span style={{fontSize:"12px",fontWeight:"500",color:tx.type==="TRANSFER"?"#a78bfa":tx.type==="ADMIN_FUNDING"||tx.type==="DEPOSIT"?"#22c55e":"#d4af37"}}>{tx.type==="TRANSFER"?"Sent":tx.type==="ADMIN_FUNDING"?"Credited":tx.type==="DEPOSIT"?"Received":tx.type}</span>{tx.description&&<p style={{fontSize:"11px",color:"#6b7280"}}>{tx.description}</p>}</div>
        ...
        {tab==="profile" && (
          <div style={{background:BG,backdropFilter:"blur(24px)",border:BR,borderRadius:"16px",padding:"24px",maxWidth:"500px"}}>
            ...
            <div style={{marginBottom:"12px"}}><label style={{fontSize:"12px",color:"#6b7280",display:"block",marginBottom:"4px"}}>KYC Status</label><div style={{padding:"12px",background:"rgba(255,255,255,0.05)",borderRadius:"10px",fontSize:"14px"}}><span style={{color:kyc==="VERIFIED"?"#22c55e":kyc==="PENDING"?"#facc15":"#ef4444"}}>{kyc}</span></div></div>
            <div style={{marginBottom:"12px"}}><label style={{fontSize:"12px",color:"#6b7280",display:"block",marginBottom:"4px"}}>Member Since</label><div style={{padding:"12px",background:"rgba(255,255,255,0.05)",borderRadius:"10px",fontSize:"14px"}}>{new Date().toLocaleDateString()}</div></div>
          </div>
        )}
        ...
        {tab==="refer" && (
          <div style={{background:BG,backdropFilter:"blur(24px)",border:BR,borderRadius:"16px",padding:"24px",maxWidth:"500px"}}>
            ...
            <div style={{background:"rgba(212,175,55,0.08)",border:"1px solid rgba(212,175,55,0.15)",borderRadius:"12px",padding:"16px",marginBottom:"16px",textAlign:"center"}}>
              <p style={{fontSize:"12px",color:"#6b7280",marginBottom:"4px"}}>Your Referral Code</p>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px",marginBottom:"16px"}}>
              <div style={{background:"rgba(255,255,255,0.03)",borderRadius:"10px",padding:"16px",textAlign:"center"}}><div style={{fontSize:"20px",fontWeight:"bold",color:"#60a5fa"}}>3</div><div style={{fontSize:"11px",color:"#6b7280",marginTop:"4px"}}>Active Referrals</div></div>
              <div style={{background:"rgba(255,255,255,0.03)",borderRadius:"10px",padding:"16px",textAlign:"center"}}><div style={{fontSize:"20px",fontWeight:"bold",color:"#22c55e"}}>$250.00</div><div style={{fontSize:"11px",color:"#6b7280",marginTop:"4px"}}>Referral Income</div></div>
            </div>
            <p style={{fontSize:"12px",color:"#6b7280"}}>Share your code with friends. You earn 5% of their first deposit!</p>
          </div>
        )}
          ...
          <div style={{maxWidth:"500px"}}>
            ...
            <div style={{background:BG,backdropFilter:"blur(24px)",border:BR,borderRadius:"16px",padding:"24px"}}>
              ...
              <form onSubmit={changePw} style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                ...
                <input type="password" value={newPw} onChange={e=>setNP(e.target.value)} placeholder="New password (min 6 chars)" style={IP} required />
