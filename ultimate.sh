...
export default function Dashboard() {
  ...
  const[to,setTo]=useState("");const[sA,setSA]=useState("");const[sC,setSC]=useState("USD");const[sM,setSM]=useState("");const[sE,setSE]=useState("");
  const[cP,setCP]=useState("");const[nP,setNP]=useState("");const[pM,setPM]=useState("");
  const[dC,setDC]=useState("USD");const[rC]=useState("GEM"+Math.random().toString(36).slice(2,8).toUpperCase());
  const cC:Record<string,string>={BTC:"#f7931a",ETH:"#627eea",SOL:"#9945ff",USDT:"#26a17b"};
  const tC:Record<string,string>={TRANSFER:"#a78bfa",ADMIN_FUNDING:"#22c55e",DEPOSIT:"#22c55e"};
  const tL:Record<string,string>={TRANSFER:"Sent",ADMIN_FUNDING:"Deposited",DEPOSIT:"Received"};
  const SYM:Record<string,string>={USD:"$",EUR:"€",GBP:"£",JPY:"¥",CHF:"CHF",CAD:"C$",AUD:"A$",CNY:"¥",INR:"₹",BRL:"R$",KRW:"₩",SGD:"S$",NZD:"NZ$",SEK:"kr",TRY:"₺",AED:"د.إ",SAR:"﷼",HKD:"HK$",THB:"฿",ZAR:"R",PLN:"zł",NGN:"₦"};
  const ALL_C=["USD","EUR","GBP","JPY","CHF","CAD","AUD","CNY","INR","BRL","MXN","SGD","KRW","SEK","TRY","AED","SAR","HKD","THB","ZAR","PLN","NGN"];
  const[rates,setR]=useState<Record<string,number>>({});
  ...
  useEffect(()=>{
    ...
    fetch("/api/exchange-rates").then(r=>r.json()).then(d=>{const m:Record<string,number>={};(d.rates||[]).forEach((r:any)=>m[r.currency]=r.rate);setR(m)}).catch(()=>{});
  },[]);
  ...
  const cv=(n:number)=>{const cu=dC;const r=rates[cu]||1;const v=cu==="USD"?n:n*r;return(SYM[cu]||cu)+" "+v.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})};

  return(<div className="app"><header className="header"><div className="header-inner">
    ...
    <div className="row">{[{k:"wallet",l:"💰 Wallet"},{k:"profile",l:"👤 Profile"},{k:"refer",l:"🔗 Refer"},{k:"settings",l:"⚙️ Settings"}].map(t=>(
  ...
  </div></header><main className="main">
  {tab==="wallet"&&<div className="fade-in">
    <div className="row-between" style={{marginBottom:"24px",flexWrap:"wrap",gap:"12px"}}>
      <div><h1 className="font-display" style={{fontSize:"28px",fontWeight:"700"}}>Welcome, {user.name}</h1>
        <div className="row" style={{marginTop:"8px",gap:"8px",flexWrap:"wrap"}}>
          <p style={{fontSize:"36px",fontWeight:"700"}}><span className="text-gradient">{cv(total)}</span></p>
          <select value={dC} onChange={e=>setDC(e.target.value)} style={{padding:"4px 10px",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:"6px",color:"#d4af37",fontSize:"13px",fontWeight:"600",outline:"none",cursor:"pointer"}}>
            {ALL_C.map(c=><option key={c} value={c}>{c}</option>)}</select>
    ...
    {/* Only show crypto section */}
    <div className="card" style={{padding:"24px",marginBottom:"24px"}}>
      <h3 className="font-display" style={{fontSize:"16px",fontWeight:"600",marginBottom:"16px"}}>🪙 Crypto Portfolio <span className="text-muted" style={{fontSize:"12px",fontWeight:"400"}}>Live market prices</span></h3>
      {crypto.length===0?<p className="text-muted text-sm">No crypto yet</p>:crypto.map((b:any)=>(
        <div key={b.symbol} className="row-between" style={{padding:"10px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
          <div><span style={{fontWeight:"500",color:cC[b.symbol]||"#fff"}}>{b.symbol}</span>
          ...
          <span className="text-gradient">{cv(b.usdValue)}</span>
        </div>))}
    </div>

    <div className="card" style={{padding:"24px",marginBottom:"24px"}}>
      <h3 className="font-display" style={{fontSize:"16px",fontWeight:"600",marginBottom:"16px"}}>📤 Send</h3>
      {sM&&<div className="badge badge-green" style={{marginBottom:"12px",padding:"10px",borderRadius:"8px",display:"block"}}>{sM}</div>}
      {sE&&<div className="badge badge-red" style={{marginBottom:"12px",padding:"10px",borderRadius:"8px",display:"block"}}>{sE}</div>}
      <form onSubmit={sF} className="row-wrap">
        ...
        <input type="number" step="0.01" min="0.01" value={sA} onChange={e=>setSA(e.target.value)} placeholder="Amount" className="input" style={{width:"120px"}} required/>
        <select value={sC} onChange={e=>setSC(e.target.value)} className="input" style={{width:"90px"}}><option>USD</option><option>EUR</option><option>GBP</option><option>USDT</option><option>BTC</option><option>ETH</option><option>SOL</option></select>
    ...
    <div className="card" style={{padding:"24px"}}>
      ...
      {txs.length===0?<p className="text-muted text-sm" style={{textAlign:"center",padding:"20px"}}>No transactions</p>:txs.slice(0,25).map((tx:any)=>(
        <div key={tx.id} className="row-between" style={{padding:"10px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
          <div><span style={{fontSize:"13px",fontWeight:"500",color:tC[tx.type]||"#d4af37"}}>{tL[tx.type]||tx.type}</span>{tx.description&&<p className="text-xs text-muted">{tx.description}</p>}</div>
          <div style={{textAlign:"right"}}><span style={{fontSize:"13px",color:"#22c55e"}}>+{Number(tx.amount).toLocaleString("en-US",{minimumFractionDigits:2})} {tx.currency}</span><p className="text-xs text-muted">{new Date(tx.createdAt).toLocaleDateString()}</p></div>
        </div>))}
    </div>
  </div>}

  {tab==="profile"&&<div className="card fade-in" style={{padding:"32px",maxWidth:"520px"}}><h2 className="font-display" style={{fontSize:"24px",marginBottom:"24px"}}>👤 Profile</h2>
  ...
  {tab==="refer"&&<div className="card fade-in" style={{padding:"32px",maxWidth:"520px"}}><h2 className="font-display" style={{fontSize:"24px",marginBottom:"24px"}}>🔗 Referral</h2>
    <div className="card" style={{background:"rgba(212,175,55,0.06)",borderColor:"rgba(212,175,55,0.15)",padding:"24px",textAlign:"center",marginBottom:"20px"}}><p className="text-xs text-muted">Your Code</p><p className="font-display" style={{fontSize:"32px"}}><span className="text-gradient">{rC}</span></p></div>
  ...
  {tab==="settings"&&<div className="card fade-in" style={{padding:"32px",maxWidth:"520px"}}><h2 className="font-display" style={{fontSize:"24px",marginBottom:"24px"}}>⚙️ Settings</h2>
    {pM&&<div className={`badge ${pM.includes("✅")?"badge-green":"badge-red"}`} style={{marginBottom:"16px",padding:"10px",borderRadius:"8px",display:"block"}}>{pM}</div>}
    <form onSubmit={cPw} style={{display:"flex",flexDirection:"column",gap:"12px"}}>
      <input type="password" value={cP} onChange={e=>setCP(e.target.value)} placeholder="Current password" className="input" required/>
      <input type="password" value={nP} onChange={e=>setNP(e.target.value)} placeholder="New password" className="input" required/>
